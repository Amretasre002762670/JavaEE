 ...
 */
public class BrowseMoviesController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getParameter("index").equalsIgnoreCase("formsubmit")) {
            ...
            String search_word = request.getParameter("searchWord");
            String search_key = request.getParameter("searchKey");
            
            request.setAttribute("word", search_word);
            request.setAttribute("key", search_key);
            
            RequestDispatcher rd = request.getRequestDispatcher("/JSP/displayMovies.jsp");
            rd.forward(request, response);
        }
    }
